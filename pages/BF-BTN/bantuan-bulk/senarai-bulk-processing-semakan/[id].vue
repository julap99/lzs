<template>
  <div>
    ...
    <div class="space-y-4 mt-4">
      ...
      <!-- Maklumat Bantuan Section (uses same edit state as the first card) -->
      ...
      <!-- <rs-card>
        <template #header>
          <div class="flex items-center space-x-3">
            ...
            <div>
              <h2 class="text-xl font-semibold text-gray-900">Dokumen Berkaitan</h2>
...
const breadcrumb = ref([
  ...
  {
    name: 'Senarai Bulk Processing Semakan',
    type: 'link',
    path: '/BF-BTN/bantuan-bulk/senarai-bulk-processing-semakan',
  },
  {
    name: `Lihat Bulk Processing Semakan ${id}`,
    type: 'current',
    path: `/BF-BTN/bantuan-bulk/senarai-bulk-processing-semakan/${id}`,
  },
])
...
const getBantuanData = (id) => {
  const data = {
    ...
    'BP-2025-01589': {
      ...
      productPackage: '(HQ) ELAUN ISBAT AL-QURAN - ELAUN PELAJAR (FISABILILLAH)',
      productEntitlement: '(HQ) ELAUN ISBAT AL-QURAN - ELAUN PELAJAR (FISABILILLAH)',
...
// Options bound to the *edit* card
const aidProductOptions_Edit = computed(() => {
  if (!bantuanData.value.aid || !bantuanData.value.bantuan) {
    return [{ label: '-- Pilih --', value: '', disabled: true }]
  }
  const node = bantuanData.value.bantuan[bantuanData.value.aid]
  if (!node) return [{ label: '-- Pilih --', value: '', disabled: true }]
  const opts = Object.keys(node).map(k => ({ label: k, value: k }))
  return [{ label: '-- Pilih --', value: '', disabled: true }, ...opts.sort((a,b) => a.label.localeCompare(b.label))]
})

const productPackageOptions_Edit = computed(() => {
  if (!bantuanData.value.aid || !bantuanData.value.aidProduct || !bantuanData.value.bantuan) {
    return [{ label: '-- Pilih --', value: '', disabled: true }]
  }
  const node = bantuanData.value.bantuan[bantuanData.value.aid]?.[bantuanData.value.aidProduct]
  if (!node) return [{ label: '-- Pilih --', value: '', disabled: true }]
  const opts = Object.keys(node).map(k => ({ label: k, value: k }))
  return [{ label: '-- Pilih --', value: '', disabled: true }, ...opts.sort((a,b) => a.label.localeCompare(b.label))]
})

const productEntitlementOptions_Edit = computed(() => {
  const aidNode = bantuanData.value.bantuan?.[bantuanData.value.aid]
  const productNode = aidNode?.[bantuanData.value.aidProduct]
  const entitlements = productNode?.[bantuanData.value.productPackage] || []
...
const handleEdit = () => {
  ...
  originalData.value = { ...bantuanData.value };
...
const handleCancel = () => {
  ...
  bantuanData.value = { ...originalData.value };
  isEditing.value = false;
};
