      ...
      <template #header>
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-semibold">
            Senarai Permohonan untuk Semakan & Sokongan (EOAD)
          </h2>
        </div>
      </template>
...
const breadcrumb = ref([
  ...
  {
    name: "Semakan & Sokongan (Panel Hartanah)",
    type: "current",
    path: "/BF-BTN/permohonan/semakan-sokongan",
  },
]);
...
// Options for filters
const statusOptions = [{ label: "Untuk Semakan", value: "untuk_semakan" }];

// State
const searchQuery = ref("");
const filters = ref({
  status: "untuk_semakan", // Default to "Untuk Siasatan"
});
...
const handleViewDetails = (noRujukan) => {
  navigateTo(`/BF-BTN/permohonan/semakan-sokongan-1/${noRujukan}`);
};

const getStatusVariant = (status) => {
  const variants = {
    untuk_semakan: "warning",
  };
