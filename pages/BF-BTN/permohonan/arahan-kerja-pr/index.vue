      ...
      <template #header>
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-semibold">
            Senarai Arahan Kerja PR (Pegawai ETD)
          </h2>
        </div>
      </template>
...
// Options for filters
const statusOptions = [{ label: "Arahan Kerja", value: "arahan_kerja" }];

// State
const searchQuery = ref("");
const filters = ref({
  status: "arahan_kerja", // Default to "Untuk Siasatan"
});
...
const handleViewDetails = (noRujukan) => {
  navigateTo(`/BF-BTN/permohonan/arahan-kerja-pr/${noRujukan}`);
};

const getStatusVariant = (status) => {
  const variants = {
    arahan_kerja: "warning",
  };
