<template>
  <div>
    <h1 class="text-xl font-semibold mb-4">Laporan Aging Bantuan</h1>
      ...
      <div class="p-4">
        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormKit
            ...
            type="select"
            label="Daerah"
            :options="['Semua Daerah', ...opsiDaerah]"
          ...
          <FormKit
            ...
            type="select"
            label="Kariah"
            :options="['Semua Kariah', ...kariahByDaerah]"
        ...
        <div class="mt-4">
          ...
          <div class="flex items-center gap-3">
            <FormKit v-model="criteria.tarikhDari" name="tarikhDari" type="date" :label="null" input-class="w-full" />
            <span class="text-slate-400">—</span>
            <FormKit v-model="criteria.tarikhHingga" name="tarikhHingga" type="date" :label="null" input-class="w-full" />
          </div>
      ...
      <rs-card>
        <div class="p-4">
          <div class="flex items-center justify-between">
            <h3 class="text-base font-semibold">Laporan Aging Bantuan</h3>
                ...
                <tbody>
                  <tr v-for="(r, i) in rows" :key="r.idBantuan" class="text-sm">
                    ...
                    <td class="px-3 py-2 border whitespace-nowrap">
                      {{ r.tarikhTamat ? formatDate(r.tarikhTamat) : '—' }}
                    </td>
...
<script setup>
definePageMeta({ title: 'Laporan Aging Bantuan' })
import { reactive, ref, computed } from 'vue'

/* Header */
const jenisLaporan = 'DOAA'
const namaLaporan  = 'Laporan Aging Bantuan'
...
const kariahByDaerah = computed(() =>
  criteria.daerah && criteria.daerah !== 'Semua Daerah' ? (petaKariah[criteria.daerah] || []) : [...new Set(Object.values(petaKariah).flat())]
)
...
/* Actions */
function onReset () {
  criteria.daerah = 'Semua Daerah'
  criteria.kariah = 'Semua Kariah'
  criteria.tarikhDari = ''
...
function onSearch () {
  ...
  rows.value = DATA_AGING.filter(r => {
    ...
    if (criteria.daerah !== 'Semua Daerah' && r.daerah !== criteria.daerah) return false
    if (criteria.kariah !== 'Semua Kariah' && r.kariah !== criteria.kariah) return false
...
function onExport (type) {
  console.log('Export:', type, { ...criteria, jenisLaporan, namaLaporan })
}
