<template>
  <div>
    ...
    <RsCard class="mt4">
      ...
      <template #body>
        <!-- Status Table -->
        <rs-table class="mt-4" :key="tableKey" :data="tableRows" :fields="fields" :pageSize="10" :options="{
          variant: 'default',
          hover: true,
        }">
      ...
      <!-- Lihat Modal -->
      <rs-modal v-model="showModal" size="lg" class="flex items-center justify-center">
        <template #header>
        ...
        <template #footer>
          <div class="flex items-center justify-end gap-2">
            <rs-button variant="primary-outline" @click="showModal = false"><Icon name="iconamoon:arrow-left-1" />Kembali</rs-button>
          </div>
        </template>
...
const breadcrumb = ref([
  ...
  },
  {
    ...
    type: "current",
    path: "/BF-PRF/KF/SHI/01_01/admin",
  },
]);
...
onMounted(() => {
  try {
    ...
    if (!Array.isArray(rows.value) || rows.value.length === 0) {
      const seed = [
        { kod: "F", nama: "Fakir", jenis: "Household", tarikhMula: "2026-01-01", statusData: "Draf" },
        { kod: "M", nama: "Miskin", jenis: "Household", tarikhMula: "2026-01-01", statusData: "Lulus" },
        { kod: "NFM", nama: "NonFM", jenis: "Household", tarikhMula: "2026-01-01", statusData: "Draf" },
        { kod: "G", nama: "Gharimin", jenis: "Individu", tarikhMula: "2026-01-01", statusData: "Tolak" },
        { kod: "F", nama: "Fakir", jenis: "Individu", tarikhMula: "2026-01-01", statusData: "Draf" },
        { kod: "M", nama: "Miskin", jenis: "Individu", tarikhMula: "2026-01-01", statusData: "Tolak" },
      ];
      rows.value = ensureIds(seed).rows; 
      persistRows();
    }
  } catch (e) {
...
function formatPct(val) {
  ...
  // clamp to your range if you want (0..9999.99)
