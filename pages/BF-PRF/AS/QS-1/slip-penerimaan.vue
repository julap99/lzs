...
onMounted(async () => {
  try {
    ...
    if (storedApplication) {
      ...
      handoverData.value = appData.handoverData || null;

      // Check if cash has been distributed
      if (appData.status !== "Telah Diagih") {
        console.log("onMounted: Cash not distributed, redirecting"); // Debug log
        await navigateTo("/BF-PRF/AS/QS-1");
        return;
      }

      // Generate slip number and initialize data
      receiptData.value.slipNumber = "SLP-" + Date.now().toString().slice(-8);
      receiptData.value.receiptDate = formatDateTimeLocal(new Date());
      receiptData.value.witnessName =
        appData.handoverData?.catatanEkp || "EKP-001";
      receiptData.value.printedBy = "System Auto";

      console.log("onMounted: Receipt data initialized:", receiptData.value); // Debug log
    } else {
  ...
  // Initialize signature canvas if using digital signature
  nextTick(() => {
...
const getApplicantName = () => {
  return (
    ...
    applicationData.value?.profileData?.nama ||
    applicationData.value?.applicantProfile?.nama ||
    "Tidak tersedia"
  );
};
...
const getApplicantIC = () => {
  return (
    ...
    applicationData.value?.profileData?.noKp ||
    applicationData.value?.applicantProfile?.noKp ||
    "Tidak tersedia"
  );
};
...
const getAssistanceType = () => {
  return applicationData.value?.jenisBantuan || "Bantuan Kecemasan";
};

const getCashAmount = () => {
  return (
    handoverData.value?.jumlahTunaiDiagih ||
    applicationData.value?.approvalData?.jumlahDisetujui ||
    "0"
  );
};

const getHandoverLocation = () => {
  return handoverData.value?.lokasiSerahan || "Tidak tersedia";
};

const getHandoverDateTime = () => {
  return handoverData.value?.tarikhMasa || new Date().toISOString();
};
                ...
                <div class="nas-action-buttons">
                  ...
                  >
                    <Icon
                      ...
                      class="nas-button-icon"
                    />
                    {{ isSubmitting ? "Memproses..." : "Sahkan Penerimaan" }}
                  </RsButton>
                </div>
