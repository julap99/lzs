<!-- 
  ...
  ROUTE: /BF-PA/PP/PD/eksekutif/detail/[rujukan]
-->
<template>
  <div class="space-y-6">
    ...
    <section class="mt-4 space-y-6">
      <header class="flex flex-wrap items-center justify-between gap-4">
        <div class="space-y-1">
          <h1 class="text-2xl font-semibold text-slate-900">Sokongan Eksekutif</h1>
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <rs-badge :variant="getSupportStatusVariant(supportData.statusSokongan)">
              {{ supportData.statusSokongan || "Belum bermula" }}
            </rs-badge>
          </div>
        </div>
      </div>
      ...
      <div class="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(260px,1fr)]">
        ...
        <div class="space-y-5">
          ...
          <!-- Borang Keputusan Sokongan -->
          <article class="rounded-lg border border-slate-200 bg-white p-4 space-y-6">
            <header class="flex flex-wrap items-start justify-between gap-3">
              <div class="space-y-1">
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Keputusan Sokongan</p>
                <h3 class="text-base font-semibold text-slate-900">Sokongan Eksekutif</h3>
              </div>
              <div class="flex items-center gap-3">
                <rs-badge
                  v-if="supportForm.statusSokongan"
                  :variant="getSupportStatusVariant(supportForm.statusSokongan)"
                  class="text-xs"
                >
                  {{ supportForm.statusSokongan }}
                </rs-badge>
                <div class="text-right text-xs text-slate-400">
                  <p class="text-[10px] uppercase tracking-wider">Tarikh</p>
                  <p class="font-medium text-slate-600">{{ displayDate(supportForm.tarikhSokongan) || currentDateDisplay }}</p>
                </div>
              </div>
            </header>
            ...
            <div class="grid gap-6 lg:grid-cols-[minmax(0,220px)_minmax(0,1fr)]">
              ...
              <section aria-labelledby="keputusan" role="radiogroup" class="space-y-3">
                ...
                <div class="grid gap-3">
                  <label
                    ...
                    :class="supportForm.statusSokongan === option.value
                      ? option.activeClass
                  ...
                  >
                    <input
                      ...
                      :aria-checked="supportForm.statusSokongan === option.value"
                    />
                    ...
                    <Icon v-if="supportForm.statusSokongan === option.value" name="ph:check" class="h-4 w-4 text-emerald-500" />
                  </label>
                </div>
              </section>
              ...
              <section class="space-y-3 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                <div class="flex items-start justify-between gap-3">
                  <div class="space-y-1">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Catatan Sokongan</p>
                  </div>
                  <span class="text-[11px] text-slate-400">{{ (supportForm.catatanSokongan && supportForm.catatanSokongan.length) || 0 }}/200</span>
                </div>
                <FormKit
                  type="textarea"
                  rows="4"
                  ...
                  placeholder="Contoh: Calon disokong untuk pengesahan ketua jabatan."
                  :validation="supportForm.statusSokongan === 'Tidak Sokong' ? 'required' : ''"
                  :validation-messages="{ required: 'Catatan diperlukan apabila status Tidak Sokong' }"
                  v-model.trim="supportForm.catatanSokongan"
                  :classes="{
            ...
            <section class="rounded-xl border border-dashed border-slate-300 bg-slate-50/50 p-4 text-xs text-slate-500">
              <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <span v-if="supportForm.additionalDocuments.length" class="text-[11px] text-slate-400">
                    {{ supportForm.additionalDocuments.length }} fail dipilih
                  </span>
                </div>
              </div>
              <input
              ...
              />
              <ul v-if="supportForm.additionalDocuments.length" class="mt-3 space-y-2">
                <li
                  v-for="(f, i) in supportForm.additionalDocuments"
                  :key="i"
            ...
            <footer class="flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 pt-4">
              ...
              <rs-button
                type="button"
                variant="primary"
                :disabled="isSubmitting || !supportForm.statusSokongan || (supportForm.statusSokongan === 'Tidak Sokong' && !supportForm.catatanSokongan)"
        ...
        <div class="space-y-4 text-center">
          ...
          <div class="space-y-2">
            ...
            <p class="text-sm text-slate-500">
              Anda akan menghantar keputusan sokongan eksekutif untuk permohonan ini. Tindakan ini tidak boleh dibatalkan.
            </p>
          </div>
          <div class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
            Keputusan: {{ supportForm.statusSokongan || "Belum dipilih" }}
          </div>
        </div>
      </template>
      <template #footer>
    ...
    <!-- Success Modal -->
    <rs-modal v-model="showSuccessModal" title="Sokongan Berjaya" size="sm">
      <template #body>
        <div class="space-y-4 text-center">
          <Icon name="ph:check-circle" class="mx-auto h-12 w-12 text-emerald-500" />
          <div class="space-y-2">
            <h3 class="text-lg font-semibold text-slate-900">Sokongan Eksekutif Berjaya</h3>
            <p class="text-sm text-slate-500">
              Keputusan sokongan anda telah berjaya dihantar dan direkodkan dalam sistem.
            </p>
          </div>
          <div class="space-y-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
            <p><strong>Status:</strong> {{ supportForm.statusSokongan }}</p>
            <p><strong>Tarikh:</strong> {{ displayDate(supportForm.tarikhSokongan) }}</p>
          </div>
        </div>
      </template>
      <template #footer>
...
const breadcrumb = ref([
  ...
  { name: "Eksekutif", type: "link", path: "/BF-PA/PP/PD" },
  { name: "Maklumat Terperinci", type: "current", path: `/BF-PA/PP/PD/eksekutif/detail/${route.params.rujukan}` },
]);
...
// Sejarah Semakan (hingga eksekutif semasa)
const sejarahSemakan = computed(() => ([
  ...
  },
  {
    ...
    tindakan: "Sokongan",
    status: "Dalam Proses",
    catatan: "Menunggu keputusan sokongan",
    dokumen: [],
  },
]));
...
const handleSubmit = async () => {
  try {
    ...
    const payload = {
      ...
      keputusan: supportForm.value.statusSokongan,
      catatan: (supportForm.value.catatanSokongan || "").trim(),
      tarikhSokongan: fmtIso(),
      disokongOleh: currentUser.value.id,
    };

    // Sediakan FormData untuk dokumen
    const formData = new FormData();
    formData.append("payload", JSON.stringify(payload));
    (supportForm.value.additionalDocuments || []).forEach((f, i) => {
      formData.append(`files[${i}]`, f, f.name);
    });

    // TODO: Panggil API sebenar
    // await $fetch('/api/pa/pradaftar/sokongan-eksekutif', { method: 'POST', body: formData });

    // Kemas kini state paparan
    supportData.value.statusSokongan = payload.keputusan;
    supportData.value.tarikhSokongan = payload.tarikhSokongan;
    supportData.value.disokongOleh = currentUser.value.name;
    supportData.value.catatanSokongan = payload.catatan;

    // Selarikan status pendaftaran:
    // Jika "Sokong" -> Disokong Eksekutif; jika "Tidak Sokong" -> Ditolak
    application.value.statusPendaftaran = payload.keputusan === "Sokong" ? "Disokong Eksekutif" : "Ditolak";
...
const handleSupportingFiles = (event) => {
  ...
  supportForm.value.additionalDocuments = safe;
  if (event?.target) event.target.value = "";
};
const removeSupportingFile = (idx) => {
  supportForm.value.additionalDocuments.splice(idx, 1);
};
...
onMounted(() => {
  supportForm.value.tarikhSokongan = fmtIso();
