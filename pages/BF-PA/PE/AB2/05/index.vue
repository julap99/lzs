    ...
    <rs-card class="mt-4">
      <template #header>
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-semibold">
            Paparan Permohonan Elaun Sedang Proses
          </h2>
          <rs-badge :variant="getStatusVariant(currentStatus)">
      ...
      <template #body>
        <div class="p-4">
          ...
          <div class="mb-6">
            <h3 class="text-lg font-semibold mb-4">Maklumat Status Semasa</h3>
            <div :class="getStatusInfoClass()">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p :class="getStatusTextClass()">Status Semasa:</p>
                  <p class="font-medium">{{ getStatusLabel(currentStatus) }}</p>
                </div>
                <div>
                  <p :class="getStatusTextClass()">Langkah Seterusnya:</p>
                  <p class="font-medium">{{ getNextStep(currentStatus) }}</p>
                </div>
                <div v-if="currentStatus === 'Menunggu Pengesahan'">
                  <p :class="getStatusTextClass()">Dihantar oleh:</p>
                  <p class="font-medium">{{ application.eksekutifApprover }} (Eksekutif)</p>
                </div>
                <div v-if="currentStatus === 'Menunggu Pengesahan'">
                  <p :class="getStatusTextClass()">Tarikh Hantar:</p>
                  <p class="font-medium">{{ application.eksekutifApprovalDate }}</p>
                </div>
                <div v-if="currentStatus === 'Menunggu Kelulusan'">
                  <p :class="getStatusTextClass()">Disahkan oleh:</p>
                  <p class="font-medium">{{ application.ketuaJabatanApprover }} (Ketua Jabatan)</p>
                </div>
                <div v-if="currentStatus === 'Menunggu Kelulusan'">
                  <p :class="getStatusTextClass()">Tarikh Sahkan:</p>
                  <p class="font-medium">{{ application.ketuaJabatanApprovalDate }}</p>
                </div>
                </div>
              </div>
            </div>
          ...
          <div class="mb-6 p-6 border border-gray-200 rounded-lg">
            ...
            <div class="space-y-4">
              ...
              <div v-if="currentStatus === 'Menunggu Kelulusan'" class="flex items-start justify-between p-4">
                <div class="flex items-start">
...
definePageMeta({
  title: "Paparan Permohonan Elaun Sedang Proses",
  description: "View processing allowance applications",
});
...
const breadcrumb = ref([
  ...
  {
    name: "Paparan Permohonan Sedang Proses",
    type: "current",
    path: "/BF-PA/PE/AB2/05",
  },
]);

// Get status from route query or default
const currentStatus = ref(route.query.status || 'Menunggu Pengesahan');
...
const getNextStep = (status) => {
  switch (status) {
    case 'Menunggu Pengesahan':
      return 'Ketua Jabatan akan menyemak dan mengesahkan'
    case 'Menunggu Kelulusan':
      return 'Ketua Divisyen akan memberikan kelulusan akhir'
    default:
...
// Load data on mount
onMounted(() => {
  // Update status from route query if available
  if (route.query.status) {
    currentStatus.value = route.query.status;
  }
  console.log('Loading processing view screen with status:', currentStatus.value);
});
</script>
